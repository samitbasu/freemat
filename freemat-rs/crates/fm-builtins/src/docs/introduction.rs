...
- Build and install Qt 4.3 or later - `http://trolltech.com/developer/downloads/opensource`

- Install g77 or gfortran (use fink for Mac OS X, use `gcc-g77` package for MinGW)

- Download the source code `FreeMat-\<VERSION_NUMBER\>-src.tar.gz`.

- Unpack the source code: `tar xvfz FreeMat-\<VERSION_NUMBER\>-src.tar.gz`.

- For Windows, you will need to install MSYS as well as MINGW to
...
- If you are extraordinarily lucky (or prepared), you can issue the
...
- I assume that you are familiar with the process of installing
